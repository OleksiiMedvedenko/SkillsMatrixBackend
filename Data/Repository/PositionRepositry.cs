...
namespace Data.Repository
{
    public class PositionRepositry : DatabaseProviderController, IPositionRepositry
    {
        ...
        public async Task<ExternalDataResultManager<bool>> ChangeStatusEmployeePositionAsync(int? areaId, int? employeeId, ActivationStatus? status)
        {
            bool commandResult = false;

            var sqlQuery = @"UPDATE ps SET ps.isActive = @status
							FROM [dbo].[Positions] AS ps
							INNER JOIN [dbo].[Position] AS p ON p.positionID = ps.positionID
							WHERE p.areaID = @areaId AND ps.employeeID = @employeeId";
