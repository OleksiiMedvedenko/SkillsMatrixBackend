...
namespace Data.Repository
{
    public class EmployeeRepository : DatabaseProviderController, IEmployeeRepository
    {
        ...
        public async Task<ExternalDataResultManager<bool>> CreateEmployeeAsync(EmployeeCreateModel? employee)
        {
            var commandResult = false;

            var sqlQuery = @"INSERT INTO Employee(firstName, lastName, supervisorID, departamentID, isActive, login) OUTPUT @positionId, INSERTED.employeeID, 1
																		INTO [dbo].[Positions](positionID, employeeId, isActive)
        ...
        public async Task<ExternalDataResultManager<IEnumerable<EmployeeViewModel>>> GetEmployeesAsync()
        {
            ...
            var sqlQuery = @"SELECT e.employeeID, e.firstName, e.lastName, e.isActive, s.employeeID, s.firstName, s.lastName, p.positionName, d.departamentID, d.departamentName, e.login, MAX(p.positionID) as positionID, MAX(permission.permissionID) as permissionID, MAX(permission.permissionName) as permissionName
                            ...
                            INNER JOIN Positions as ps ON ps.employeeID = e.employeeID
                            INNER JOIN Position as p ON ps.positionID = p.positionID
                            INNER JOIN Departament as d ON e.departamentID = d.departamentID
                            ...
                            GROUP BY e.employeeID, e.firstName, e.lastName, e.isActive, s.employeeID, s.firstName, s.lastName, p.positionName, d.departamentID, d.departamentName, e.login";
                ...
                using (var reader = command.ExecuteReader())
                {
                    while (await reader.ReadAsync())
                    {
                        var employee = new EmployeeViewModel(
                            ...
                            new Position(reader.GetFieldValue<int>(11), reader.GetFieldValue<string>(7)),
                            new List<Area>(),
